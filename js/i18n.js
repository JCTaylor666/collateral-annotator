...
(function () {
  ...
  const DICT = {
    en: {
      ...
      copyDiagDone: 'Copied ✓',
      noteCorruptBackedUp: '{id}: note.json was unreadable — the original was preserved as note.json.corrupt before saving.',
      noteExternalBackedUp: '{id}: the note.json already in the folder had not been read into this session, so it was kept as note.external-backup.json before your note replaced it.',
    ...
    zh: {
      ...
      copyDiagBtn: '复制诊断信息',
      copyDiagDone: '已复制 ✓',
      noteCorruptBackedUp: '{id}:note.json 无法解析 —— 保存前已把原件保留为 note.json.corrupt。',
      noteExternalBackedUp: '{id}:文件夹里原有的 note.json 本次没能读进来,已在你的笔记覆盖它之前保留为 note.external-backup.json。',
