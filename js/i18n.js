...
(function () {
  ...
  const DICT = {
    en: {
      ...
      perfSmoothLabel: 'Perfusion smoothness',
      perfMaskLabel: 'Filter by minip vessel mask',
      ...
      perfMaskUnavailable: 'minip vessel mask unavailable for this case (missing or size mismatch) — perfusion shown unfiltered.',
      shapeMismatchTitle: 'Shape mismatch — frame not loaded',
      shapeMismatchHint: 'image, segmentation and mask (if present) must share the same W × H',
      shapeMismatchBanner: '{id}: image / segmentation / mask sizes differ — shown as a placeholder, not editable.',
      smImage: 'image · frames.png',
      smLabel: 'segmentation · label.npy',
      smMask: 'mask · mask.npy',
      smNone: 'none',
      smUnreadable: 'unreadable',
      ...
      layersLabel: 'Layers',
      addLayerBtn: '+ Layer',
      addLayerTitle: 'Add a new empty layer',
      layerSwitchTitle: 'Switch to this layer',
      ...
      clickSingle: 'Single',
      clickBrush: 'Brush',
      magSnapLabel: 'Magnetic snap (grab nearest vessel)',
      magSnapHint: 'Off: click exactly on a segment to select it. On: a click near a vessel snaps onto the nearest one.',
      geomGroup: 'Segment geometry',
      geomMetricLabel: 'Metric',
      geomEnableLabel: 'Enable filter',
      geomMinLabel: 'Min',
      geomMaxLabel: 'Max',
      geomStatsFmt: '{n} segments · {metric} {min}–{max} · mean {mean} · median {med}',
      geomShowingFmt: 'showing {x} / {n} segments',
      selAdd: 'Select',
      selErase: 'Deselect',
      ...
      copyPaintSkipped: ' · paint skipped (frames differ in size)',
      ...
      errQuotaFull: 'Browser local backup is FAILING (storage full or blocked) — unsaved changes may not survive a reload. Keep auto-save on and press “Save to folder” now.',
      writeFailedBanner: 'Writing “{id}” to the folder FAILED — the work is kept in this browser and marked unsaved. Check the folder permission / disk, then press “Save to folder”.',
      confirmSwitchDirty: 'This folder still has {n} frame(s) with UNSAVED annotations. Opening a different folder will DISCARD them.\n\nOK = discard and switch    Cancel = stay (press “Save to folder” first)',
      multiTabWarn: 'This annotator appears to be open in ANOTHER tab as well. Use ONE tab only — two tabs overwrite each other’s local backup.',
    ...
    zh: {
      ...
      perfSmoothLabel: '灌注平滑度',
      perfMaskLabel: '按 minip 血管掩膜过滤',
      exportPerfusion: '导出灌注图',
      perfCap: '灌注',
      perfLegendCap: '到达帧(早 → 晚)',
      perfFailed: '无法为该病例计算灌注图(需要至少 2 张同尺寸的帧)。',
      perfMaskUnavailable: '本 case 的 minip 血管掩膜不可用（缺失或尺寸不符）—— 灌注图按未过滤显示。',
      shapeMismatchTitle: '形状不一致 — 该帧未加载',
      shapeMismatchHint: '图像、分区、mask(若存在)的 W × H 必须完全一致',
      shapeMismatchBanner: '{id}：图像 / 分区 / mask 尺寸不一致 —— 以占位图显示,不可编辑。',
      smImage: '图像 · frames.png',
      smLabel: '分区 · label.npy',
      smMask: 'mask · mask.npy',
      smNone: '无',
      smUnreadable: '无法解析',
      ...
      layersLabel: '图层',
      addLayerBtn: '+ 图层',
      addLayerTitle: '新建一个空图层',
      layerSwitchTitle: '切换到此图层',
      ...
      clickSingle: '单点',
      clickBrush: '笔刷',
      magSnapLabel: '磁性吸附(吸附最近的血管)',
      magSnapHint: '关:点击时必须正好点在段上才选中。开:点在血管附近会自动吸附到最近的段。',
      geomGroup: '分区几何',
      geomMetricLabel: '参数',
      geomEnableLabel: '启用过滤',
      geomMinLabel: '最小',
      geomMaxLabel: '最大',
      geomStatsFmt: '{n} 段 · {metric} {min}–{max} · 均值 {mean} · 中位 {med}',
      geomShowingFmt: '显示 {x} / {n} 段',
      selAdd: '选中',
      selErase: '取消',
      selRadiusLabel: '半径',
      ...
      errQuotaFull: '浏览器本地备份异常(存储已满或被禁用)—— 未保存的改动可能无法在刷新后保留。请保持自动保存开启,并立即点“保存到文件夹”。',
      writeFailedBanner: '“{id}” 写入文件夹【失败】—— 改动仍保留在浏览器中并标记为未保存。请检查文件夹权限/磁盘,然后点“保存到文件夹”。',
      confirmSwitchDirty: '当前文件夹还有 {n} 帧标注【未保存】。打开新文件夹会丢弃这些改动。\n\n确定 = 丢弃并切换    取消 = 留下(请先点“保存到文件夹”)',
      multiTabWarn: '本标注工具似乎还在另一个标签页打开着。请只用一个标签页 —— 两个标签页会互相覆盖本地备份。',
