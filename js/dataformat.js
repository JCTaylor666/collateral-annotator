...
<p class="doc-p"><b>Recovery.</b> Opening a frame that has any of these backup files shows a banner with a <i>View</i> action: a chooser compares the current version with each backup. Restoring is a TRADE, never a deletion: the version being replaced is written to a free backup slot of its own family first — the same name if that slot is empty, otherwise the next free <code>-2</code>…<code>-9</code> — so no existing backup is ever overwritten and everything stays listed in the chooser. If all nine slots of that family are taken by different content, the restore is refused rather than performed, and the banner says which files to move aside.</p>
...
<p class="doc-p"><b>恢复。</b>打开带有上述备份文件的帧会出现横幅和「查看」按钮:对比当前版本与各备份。恢复是<b>置换</b>,绝不是删除:被替换下来的那一版会先写进同一族里空着的备份槽位 —— 同名槽位空着就用同名,否则用下一个空着的 <code>-2</code>…<code>-9</code> —— 所以已有的备份一个都不会被覆盖,而且全都留在对话框里可以再换回来。如果该族九个槽位都被不同的内容占满了,恢复会被拒绝而不是执行,横幅会告诉你该把哪些文件挪走。</p>
