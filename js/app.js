...
(function () {
  ...
  // ROUND-4 REDESIGN. ANS-3's set conflated two different things — "the note file could not be read" and
  // "State's note did not come from that file" — and round 4 showed every way that hurts: a hiccup refused
  // the ANNOTATION too (OT-3), the background scan's later successful read cleared the flag even though it
  // had SKIPPED the import, so the next write replaced a colleague's note with nothing (E2E-1, data loss),
  // and a perfectly valid note was copied to note.json.corrupt as if damaged (NSL-2).
  // Membership now means exactly one thing: note.json is on disk and its content has not been taken into
  // State. It is cleared ONLY where the content is genuinely adopted.
  ...
  // Messages about ONE frame. They may sit at critical priority (they have to, to take the slot from the
  ...
  function setBanner(key, vars, kind) {
    ...
    // R9-2: remember a dataset-wide critical message displaced by a per-unit one, so clearUnitBanner can put
    // it back. A hand dismissal (key == null) means the doctor is done with it: forget it.
    if (key && lastBanner && bannerPrio(lastBanner.key, lastBanner.kind) === 2 &&
        PER_UNIT_BANNERS.indexOf(key) >= 0 && PER_UNIT_BANNERS.indexOf(lastBanner.key) < 0) displacedBanner = lastBanner;
    else displacedBanner = null;
  ...
  function asButton(el, label) {
    ...
    // ROUND-5/R5-4: ROUND-4 dropped stopPropagation() here so Space would still reach the canvas pan
    // modifier — but the two destructive controls (layer ✕, frame ☆) call stopPropagation() in their OWN
    // onclick, on this very event, so it never did. The real problem is upstream: a MOUSE click left focus
    // sitting on the span, and the Space the doctor then held to pan fired the control instead, once per
    // auto-repeat, each one a real annotation.json write. Do not take focus from a pointer press. (Chrome
    // and Edge DO focus a native button on mousedown, so this is a deliberate difference, not a
    // restoration of platform behaviour — the accidental writes are worse than the inconsistency.)
    // Tab + Enter/Space still work for keyboard users.
    el.addEventListener('mousedown', ev => { if (ev && ev.preventDefault) ev.preventDefault(); });
    el.addEventListener('keydown', ev => {
      if (ev.key !== 'Enter' && ev.key !== ' ') return;
      ev.preventDefault();     // ROUND-6/R6-3: this must come FIRST — v92 returned on auto-repeat before cancelling, so a held Space on a focused ☆ scrolled the whole page out from under the doctor
      if (ev.repeat) return;   // …but only the first press acts: one write per repeat is never what was meant
  ...
  // ROUND-5/R5-1: the frames the CURRENT savedPartial banner is about. Save-all's `failed` counter includes
  // frames whose file could not be READ — those never enter the retry queue — so "is the retry queue empty"
  // could never answer "is this banner still true". Keep the actual subjects and ask them.
  let savedPartialUnits = new Set();
  const savedPartialResolved = () => ![...savedPartialUnits].some(k => {
    const i = k.indexOf('/'); return i > 0 && State.isDirty(k.slice(0, i), k.slice(i + 1));
  });
  function bannerAddAction(labelKey, onClick) {
    ...
    act.onclick = ev => onClick(ev, act);   // ROUND-4/NSL-4: handlers get the LIVE span, never one captured in a closure that a language switch has since detached
    b.appendChild(act);
    if (view && $('view').clientHeight !== h0) onResize();
    return act;
  }
  ...
  function clearUnitBannerActions() {
    ...
    bannerActions = [];
    // ROUND-4 FIX (found independently by all six lenses — the worst thing v90 shipped): the line that used
    // to stand here ASSIGNED to `b.children`. In a real browser that is a getter-only HTMLCollection, so in
    // strict mode — which this file is — it throws TypeError, and it sits on the navigation path: the first
    // move off a frame carrying a [View backups] or [Copy diagnostic] link killed showUnit mid-flight. The
    // test double happened to expose `children` as a plain array, which is exactly why the suite stayed
    // green. removeChild/remove is the only correct way to detach, and the double now implements it.
    const b = $('banner');
    for (const c of Array.from(b.querySelectorAll('.banner-act'))) { if (c.remove) c.remove(); }
  }
  function clearUnitBanner() {
    ...
    const perUnit = PER_UNIT_BANNERS;
    // ROUND-9/R9-2: a per-unit CRITICAL message can take the slot from a dataset-wide one — a write failure
    // whose retry ladder has already given up, say — and then clear itself here, leaving the doctor with
    // nothing where a true warning stood. Put back the one it displaced.
  ...
  async function openFolderTxn() {
    ...
    try {
      ...
      // ROUND-4/R3D-2: remintDatasetId WRITES .annotator_dataset.json into the picked folder, and it ran
      // BEFORE the unsaved-work confirm — so cancelling that dialog still left a brand-new id in a folder the
      // doctor chose not to open, orphaning the crash mirror keyed to its old id. Decide here, write after
      // the cancel point, and openFolderTxn's all-or-nothing promise holds again.
      const willRemint = !!(rootHandle && newId && newId === State.getDatasetId() && !(await isSameFolder(rootHandle, newRoot)));
      if ((willRemint || newId !== State.getDatasetId()) && State.dirtyCount() > 0 &&
          !confirm(I18n.t('confirmSwitchDirty', { n: State.dirtyCount() }))) return { restore: prevView };
      // …and the WRITE itself waits until nothing can still abort (see just above the commit).
      ...
      // with {restore} or throw. A Drive listing failure therefore left a folder the doctor never opened
      // carrying a brand-new id, orphaning the crash mirror keyed to its old one. This is the LAST point
      // that can abort, so the only write into the picked folder happens after it.
      const cls = await Loader.loadClasses(newRoot);
      if (willRemint) { newId = await remintDatasetId(newRoot); reminted = true; }
      ...
      // ROUND-3 FIX: these two were the ONLY per-unit sets the wave added and forgot here, so one folder's
      // corrupt/unreadable frames silently disabled class deletion and note writing for the SAME-NAMED
      // frames of every folder opened afterwards — datasets reuse case_N/frame_M names by design.
      unimportedUnits.clear(); noteUnreadableUnits.clear();
  ...
  async function openFolder() {
    ...
    let shown = okUnit;
    if (!shown && !cur && cases.length) { for (let i = 1; i < Math.min(cases[0].units.length, 4); i++) { if (await showUnit(0, i)) { shown = true; break; } } }
    ...
    // ROUND-3 FIX: the open-time warnings were gated on `okUnit`, so when F4's fallback put a LATER frame on
  ...
  // Like rawFileText, but keeps "the file is not there" (safe to create) apart from "it is there and we
  // could not read it" (must never be overwritten blind). ROUND-4: rawFileText collapses both into null,
  // which would stall a frame whose note.json a colleague DELETED behind a stale unreadable flag.
  async function rawFileProbe(unit, name) {
    let fh;
    try { fh = await unit.handle.getFileHandle(name); }
    catch (e) { return (e && e.name === 'NotFoundError') ? { absent: true } : { failed: true }; }
    try { return { text: await (await fh.getFile()).text() }; }
    catch (e) { return (e && e.name === 'NotFoundError') ? { absent: true } : { failed: true }; }   // ROUND-5/R5-9: a colleague deleted it between the handle and the read — there is nothing left to protect, so do not stall the doctor's note
  }
  ...
  async function ensureSeeded(c, u) {
    ...
    if (r.annCorrupt || r.annDropped) corruptUnits.add(k); else { corruptUnits.delete(k); corruptBackedUp.delete(k); }   // ROUND-4/R4-GS-5: the "already backed up" flag was add-only, so a file that went corrupt, was fixed, then went corrupt AGAIN was overwritten with no backup   // keeps backupCorruptOnce's invariant — a later write must still .corrupt-backup the original (an unparseable one, or one whose layers this build cannot represent)
    ...
    else if (!r.noteUnreadable) { noteCorruptUnits.delete(k); noteCorruptBackedUp.delete(k); }   // ROUND-4/R4-GS-5: parseable again — and a LATER corruption must be preserved afresh
    if (r.noteUnreadable) noteUnreadableUnits.add(k);
    else if (rec === 'clean' || rec === 'stale-clean') noteUnreadableUnits.delete(k);   // ROUND-5/R5-2: ONLY these two outcomes adopt the file into State; 'kept-dirty'/'conflict' keep the LOCAL version and import nothing
  ...
  async function loadCur() {
    ...
    if (data.noteUnreadable) noteUnreadableUnits.add(k);   // ROUND-4/E2E-1: the CLEAR moved below, to the paths that actually take the file's note INTO State
    if (data.annCorrupt || data.annDropped) corruptUnits.add(k); else { corruptUnits.delete(k); corruptBackedUp.delete(k); }   // ROUND-4/R4-GS-5
    // ROUND-3 FIX (SWG-5 symmetry): the write guard measures max(annotation, note); this must record the
    // same quantity, or a frame whose note.json is simply newer than its annotation.json is judged conflicted for ever.
    const m = Math.max(data.annMtime || 0, data.noteMtime || 0);
    ...
    if (!unchanged) {
      ...
      else if (!data.noteUnreadable) { noteCorruptUnits.delete(k); noteCorruptBackedUp.delete(k); }   // ROUND-4/R4-GS-5
    }
    // Cleared only where State now mirrors the file's note. ROUND-5/R5-2: v91 fixed the scan but left THIS
    // clear outside the reconcile block and blind to its outcome — and 'kept-dirty' is returned by
    // reconcile's FIRST guard, before any content is compared, precisely when the local version is kept and
    // the file's note is NOT imported. One Drive hiccup, a note typed, then a step to the next frame and
    // back was enough to drop the guard and let the next save overwrite a colleague's note unseen.
    if (!data.noteUnreadable && (unchanged || rec === 'clean' || rec === 'stale-clean')) noteUnreadableUnits.delete(k);
  ...
  async function showUnit(nci, nui) {
    ...
    if (conflictedUnits.has(State.key(c.id, u.id))) {
      ...
      bannerAddAction('copyDiagBtn', async (ev, span) => { if (await copyProtectedDiag(k2) && span) span.textContent = I18n.t('copyDiagDone'); });
    }
  ...
  let classesBackedUp = false, classesBackupName = 'classes.json.corrupt';   // R9-10        // classes.json.corrupt already holds the unreadable file currently on disk
  ...
  async function backupClassesOnce() {
    ...
    try {
      ...
      // ROUND-8/R8-7: the last unconditional sidecar write in the app. classesBackedUp is cleared whenever
      // classes.json parses again (and starts false after a reload), so a SECOND, different corruption
      // overwrote the first preserved original — which existed nowhere else. The Data format page says that
      // cannot happen; now it cannot.
      const raw = await (await fh.getFile()).text();
      if (root !== rootHandle) return false;
      const slot = await freeBackupName({ handle: root }, 'classes.json.corrupt', raw);
      if (slot === null) return 'slots';   // ROUND-9/R9-6: permanent — "check the folder permission and try again" would be a lie
      if (slot.write) await FS.writeText(root, slot.name, raw);
      if (root !== rootHandle) return false;
      classesBackupName = slot.name;   // ROUND-9/R9-10: R8-7 gave this file numbered slots; the banner still named slot 1, which holds an OLDER corruption — and the doctor would restore the wrong class vocabulary for every case in the folder
  ...
  async function saveClasses() {
    ...
    const cbk = wasCorrupt ? await backupClassesOnce() : true;
    if (cbk !== true) {   // ROUND-9/R9-6: a full backup family never clears by retrying; say which files to move
      setBanner(cbk === 'slots' ? 'classesNoSlotFmt' : 'classesBackupFailed', cbk === 'slots' ? { file: 'classes.json.corrupt' } : null, 'warn');
      setSaveStatus('classesSaveFailed', null, true); return;
    }
    try {
  ...
  async function startBackgroundScan() {
    ...
    const worker = async () => {
      while (next < units.length) {
        ...
        let ann = null, note = null, annCorrupt = false, noteCorrupt = false, versionAhead = 0, annMtime = 0, annDropped = 0, unreadable = false, noteUnreadable = false;
        try { const r = await Loader.loadAnnotation(u); ann = r.annotation; note = r.note; annCorrupt = r.annCorrupt; noteCorrupt = !!r.noteCorrupt; versionAhead = r.versionAhead || 0; annMtime = r.mtime || 0; annDropped = r.annDropped || 0; unreadable = !!r.unreadable; noteUnreadable = !!r.noteUnreadable; }
        ...
        if (annCorrupt) unimportedUnits.add(uk); else unimportedUnits.delete(uk);          // UI-1 (ROUND-3: the scan only ever ADDED, so a frame that became readable stayed blocked)
        if (noteUnreadable) noteUnreadableUnits.add(uk);   // ROUND-4/E2E-1: the clear lives in the reconcile block below — this loop SKIPS the import for units loadCur already seeded, and clearing there armed the next write to wipe the file it never read
        if (annCorrupt || annDropped) corruptUnits.add(uk); else { corruptUnits.delete(uk); corruptBackedUp.delete(uk); }   // ROUND-4/R4-GS-5 (the scan only ever ADDED)
        if (!sessionLoaded.has(uk) && !writingUnits.has(uk)) { // skip units loadCur already reconciled (keeps undo history) and units a save is writing RIGHT NOW (our snapshot of their file is already stale)
          ...
          else if (!noteUnreadable) { noteCorruptUnits.delete(uk); noteCorruptBackedUp.delete(uk); }   // ROUND-4/R4-GS-5
          if (!noteUnreadable && (rec === 'clean' || rec === 'stale-clean')) noteUnreadableUnits.delete(uk);   // ROUND-5/R5-2: a reconcile that KEPT the local version imported nothing
  ...
  // ROUND-9/R9-4: v95 closed the dialog unconditionally from a resolution that may be long abandoned — so a
  // slow refusal on frame A dismissed the chooser the doctor had just opened on frame B, leaving B
  // conflicted with every write silently refused and nothing on screen about it. Touch the screen only when
  // what is on it is ours; the same rule restoreRescue already follows.
  function refuseNoSlot(k, uu, file) {
    if (conflictShownFor === k) hideConflictDialog();
    setBanner('conflictNoSlotFmt', { id: uu, file }, 'warn');
  }
  ...
  async function resolveConflictInner(k, choice, rec) {
    ...
    if (choice === 'disk') {
      ...
      try {
        ...
        const needA = State.unitHasContent(cc, uu) && annContentSig(localAnn) !== annContentSig(r.ann);
        const needN = State.hasNoteData(cc, uu) && noteContentSig(State.buildNote(cc, uu)) !== noteContentSig(r.note);
        const planA = needA ? await parkPlan(unit, 'annotation.unsaved-backup.json', () => JSON.stringify(State.buildAnnotation(cc, uu, sz[0], sz[1]), null, 2)) : null;
        if (needA && planA === null) { refuseNoSlot(k, uu, 'annotation.unsaved-backup.json'); return; }
        const planN = needN ? await parkPlan(unit, 'note.unsaved-backup.json', () => JSON.stringify(State.buildNote(cc, uu), null, 2)) : null;
        if (needN && planN === null) { refuseNoSlot(k, uu, 'note.unsaved-backup.json'); return; }
        const stillOurs = () => tok === dsToken;   // R9-8
        if (planA) { const nm = await parkWrite(unit, planA, stillOurs); if (nm === null) return; backedUp = true; keptFiles.push(nm); }
        if (planN) { const nm = await parkWrite(unit, planN, stillOurs); if (nm === null) return; backedUp = true; keptFiles.push(nm); }
      ...
      if (r.ann) {
        ...
        if (cur && cur.caseId === cc && cur.unitId === uu) {
          cur.protected = protectedUnits.has(k);
          $('note').disabled = !!cur.protected;   // ROUND-3 FIX: re-locking the frame left the note box enabled, and onNoteInput silently drops every keystroke on a protected frame — exactly the F6 hole, re-opened by F8's own fix
        }
      }
      ...
      // ROUND-8/R8-8: these land after every await, and Escape / "Decide later" let the doctor walk away in
      ...
      // ROUND-8/R8-5: the folder copy can now land in a NUMBERED slot, and the two banners below named
      // slot 1 regardless — which by construction holds different, older content. The doctor would open the
      // file the app told them held the irreplaceable bytes and find something else.
      let extAnnName = 'annotation.external-backup.json';
      try {
        ...
        const body = r.ann ? JSON.stringify(r.ann, null, 2) : await rawFileText(unit, 'annotation.json');
        if (body === null) { hideConflictDialog(); setBanner('conflictDiskUnsavable', null, 'warn'); return; }
        const planA = await parkPlan(unit, 'annotation.external-backup.json', () => body);
        if (planA === null) { refuseNoSlot(k, uu, 'annotation.external-backup.json'); return; }
        const bodyN = r.note ? JSON.stringify(r.note, null, 2) : await rawFileText(unit, 'note.json');
        const planN = bodyN === null ? null : await parkPlan(unit, 'note.external-backup.json', () => bodyN);
        if (bodyN !== null && planN === null) { refuseNoSlot(k, uu, 'note.external-backup.json'); return; }
        const stillOurs = () => tok === dsToken;   // R9-8 (these two closures are constants, but the guard costs nothing and the next edit may not be)
        const nmA = await parkWrite(unit, planA, stillOurs); if (nmA === null) return;
        extAnnName = nmA;
        if (planN && (await parkWrite(unit, planN, stillOurs)) === null) return;
      } catch (e) { setBanner('saveFailedMsg', { msg: e.message }, 'warn'); return; }
      if (tok !== dsToken) return;
      conflictedUnits.delete(k);
      // ROUND-4 FIX: v89 taught ensureSeeded/loadCur/the scan to record max(annotation, note) so the write
      // guard measures like for like — and missed this one. Recording the annotation's mtime alone here
      // meant that resolving a conflict on a frame whose note.json is newer immediately re-armed the guard:
      // the write we were about to make was refused as a fresh conflict, right after telling the doctor
      // their version had been kept.
      lastSeenMtime.set(k, (await unitDiskMtime(unit)) || 0);   // the file as it stands is KNOWN to us now — our write may proceed over it
      hideConflictDialog();
      try { await writeUnit(cc, unit); } catch (e) { }         // failures land in the retry queue like any write
      highlightNav(); updateDirtyUI();
      // ROUND-3 FIX: CONF-4's "a write that RETURNS is not a write that HAPPENED" re-check was added to
      // runAutoSave, saveNote, runRetries, toggleStar and cross-frame undo — and missed the one caller that
      // most needs it. writeUnit can refuse again here (the file moved on between the backup and the write),
      ...
      // conflictFoundFmt they answer. R8-8 suppressed them off-frame, so that warning — critical, and until
      ...
      if (protectedUnits.has(k)) {
        ...
        setBanner('conflictDiskUnreadable', { file: extAnnName }, 'warn');
        if (!retryQ.size) setSavedStatus();
      } else {
        setBanner(r.ann ? 'conflictKeptLocal' : 'conflictKeptLocalNoBackup', r.ann ? { file: extAnnName } : null, 'ok');
        if (!retryQ.size) setSavedStatus();
      }
    }
  }
  ...
  const RESCUE_NAMES = ['annotation.unsaved-backup.json', 'note.unsaved-backup.json',
    'annotation.external-backup.json', 'note.external-backup.json', 'note.unread-backup.json',
    'annotation.json.corrupt', 'note.json.corrupt'];
  // …plus the numbered siblings freeBackupName() creates for a SECOND, different corruption. A rescue file
  // the chooser cannot see is a rescue file that does not exist, so match the family, not just the name.
  // ROUND-6/R6-10: the first cut matched any digits after the prefix, so a colleague's `note.json.corrupt-0`
  // or a script's `-42` was adopted into the chooser as if this app had written it. freeBackupName only ever
  // produces `-2`…`-9` on a name that is already in the list — match exactly that, and nothing else.
  const rescueBase = n => { const m = /^(.*)-([2-9])$/.exec(n); return (m && RESCUE_NAMES.indexOf(m[1]) >= 0) ? m[1] : n; };
  const isRescueName = n => RESCUE_NAMES.indexOf(rescueBase(n)) >= 0;
  ...
  async function checkRescueFiles(c, u) {
    ...
    try {
      const names = [];
      for await (const [name, h] of u.handle.entries()) if (h.kind === 'file' && isRescueName(name)) names.push(name);
      names.sort((a, b) => (RESCUE_NAMES.indexOf(rescueBase(a)) - RESCUE_NAMES.indexOf(rescueBase(b))) || (a < b ? -1 : a > b ? 1 : 0));
      if (tok !== dsToken) return;
      ...
      // ROUND-3 FIX: riding along (R11/R8) means this can be called again for the SAME banner — every
  ...
  function rescueSwapTarget(name) {
    // ROUND-5: the numbered .corrupt siblings and the new note.unread-backup.json have the same property as
    // the originals — their whole value is content this session could not otherwise produce — so the
    // displaced version must NOT be written back over them; it goes to the .unsaved-backup sidecar.
    // ROUND-6/R6-5: …but that sidecar can already hold something real (a note parked by an earlier conflict
    // resolution), and v92 overwrote it — a restore that used to be a reversible self-swap became a one-way
  ...
  async function restoreRescue() {
    ...
    const swapBase = rescueSwapTarget(f.name);
    // ROUND-6/R6-5: BOTH branches park the version they displace, and both must land on a FREE slot — the
    ...
    // ROUND-8/R8-6: v94 closed the chooser on EVERY abandonment. Two ways that hurt. Clicking another tab
    // to compare, while a restore was still probing, silently cancelled it AND dismissed the dialog. Worse,
    // a dead attempt from a frame the doctor had already left closed the chooser they had just opened on
    // the NEW frame — and nulling rescueShownFor then aborted the restore they were in the middle of.
    // Abandon this attempt; touch the screen only when what is on it is ours.
    if (tok !== dsToken || !cur || State.key(cur.caseId, cur.unitId) !== k) {
      if (rescueShownFor === k) hideRescueDialog();
      return;
    }
    // ROUND-9/R9-7: leave what they are looking at alone — but a Restore click that does nothing at all is
    // the dead end this project keeps re-learning. Say the click was dropped, and why.
    if (rescueShownFor !== k || rescueFiles[rescueSel] !== f) { setBanner('rescueSupersededFmt', { file: f.name }, 'warn'); return; }
  ...
  async function runSave(caseId) {
    ...
    try {
      for (const k of keys) {
        ...
        const ref = map.get(k);
        if (!ref) { const [cc, uu] = k.split('/'); if (State.isDirty(cc, uu)) { failed++; failedKeys.add(k); } continue; }   // unit's folder no longer discoverable — surface it, don't silently skip
        ...
        if (st !== 'seeded' && st !== 'already') { failed++; failedKeys.add(k); continue; }   // could not read this frame's file: leave it untouched and report it, never write a State we could not verify
    ...
    if (failed) { savedPartialUnits = failedKeys; saveModalFinish(I18n.t('saveDonePartialFmt', { n, failed })); setBanner('savedPartial', { n, failed }, 'warn'); }
  ...
  // ROUND-5/R5-11: ROUND-4 re-armed these latches when a file became readable again, so a SECOND
  // corruption would be preserved — but the backup name was fixed, so the second copy simply overwrote the
  // first, and the first is by construction the only surviving copy of what the app replaced. (corruptUnits
  // also holds VALID files carrying layers this build cannot represent, so the destroyed copy can be a
  ...
  // ROUND-8/R8-4: freeBackupName performs up to nine sequential reads, and on a slow folder the doctor goes
  // on typing during them. v93 built the body and wrote it in ONE expression; v94 put those reads in
  // between, so the snapshot taken BEFORE them was parked and State.resetUnit then wiped everything typed
  // since — under a banner saying it had been preserved. Build the body again once the slot is known, and
  // write it even when the probe matched, because what the probe matched was the STALE version.
  // ROUND-9/R9-3: split into PLAN and WRITE. A resolution parks two files, and v95 wrote the first before
  // discovering the second family was full — then told the doctor "nothing was changed, no backup was
  // touched", which was false and sent them looking for a file the app had in fact just created. Resolve
  // both slots first; write only once both are certain.
  async function parkPlan(unit, base, build) {
    const probe = build();
    const slot = await freeBackupName(unit, base, probe);
    return slot === null ? null : { slot, probe, build };
  }
  // ROUND-9/R9-8 (CRITICAL): R8-4 rebuilt the body after the probes so a doctor who kept typing would not
  // lose it — but State.buildAnnotation/buildNote return an EMPTY document for a unit State no longer knows,
  // and switchDataset wipes State. Open another folder during those nine probe reads and the rebuild
  // described nothing, which then overwrote the ORIGINAL folder's backup slot with an empty document the
  // rescue chooser would happily offer back. The guard must be re-checked between the probes and the
  // rebuild, not just before them.
  async function parkWrite(unit, plan, guard) {
    if (guard && !guard()) return null;
    const body = plan.build();                 // R8-4: whatever State holds NOW, not when the probing started
    if (plan.slot.write || body !== plan.probe) await FS.writeText(unit.handle, plan.slot.name, body);
    return plan.slot.name;
  }
  ...
  // ROUND-8/R8-1: returns true | 'slots' (every backup slot taken by different content — permanent until the
  // doctor frees one) | false (a transient I/O failure — retrying really does clear it). v94 collapsed all of
  // these into one message that told the doctor to go delete rescue files that were not the problem.
  ...
  async function runRetries() {
    ...
    if (retryQ.size) { setSaveStatus('retryPending', { n: retryQ.size }, true); armRetry(RETRY_DELAYS[RETRY_DELAYS.length - 1]); }
      ...
      // ROUND-3 FIX: RS-5 widened this to savedPartial, but Save-all's `failed` counter also counts frames
      // that ensureSeeded could not READ — those never enter the retry queue, so a recovered retry of some
      // OTHER frame retired a warning that was still entirely true for them. Only retire it when the retry
      // queue draining actually accounts for every frame still unsaved.
      // ROUND-4 FIX: BAN-4 gated this on the DATASET-wide dirty count, so in any ordinary session — where
      // the doctor is still editing other frames — savedPartial could never be retired at all and went on
      // suppressing every per-frame warning for the rest of the day. What matters is narrower: are any of
      // the frames this banner is ABOUT still unwritten? Those are the ones the retry queue was carrying.
      // ROUND-5/R5-1: ROUND-4's `!retryQ.size` sat INSIDE the `else if (recovered)` arm, which is only
      // reachable once `if (retryQ.size)` has already tested false — a tautology, so savedPartial was
      // retired unconditionally, which is exactly what the ROUND-3 note three lines up says must not
      // happen. Ask the frames the banner is about.
      const partialStands = !!(lastBanner && lastBanner.key === 'savedPartial') && !savedPartialResolved();
      if (lastBanner && (lastBanner.key === 'writeFailedBanner' || (lastBanner.key === 'savedPartial' && !partialStands))) setBanner(null);
      if (partialStands) setSaveStatus(null); else setSaveStatus('saved', { time: hhmm() });   // frames from that Save-all are still unwritten: the header must not say Saved
    }
  ...
  async function writeUnitInner(caseId, unit, tok, k) {
    ...
    // ROUND-3 FIX (the worst regression this wave introduced): ANS-3's guard skipped the note WRITE but let
    // markClean run anyway — so a note the doctor had just typed was dropped, the frame was marked clean,
    // its fat mirror record went skinny and the header said "Saved". That is the exact data loss ANS-3 set
    // out to prevent, only pointed the other way. A note the doctor authored must reach disk. The original
    ...
    if (hasNote && noteUnreadableUnits.has(k)) {
      ...
      if (tok !== dsToken) return;
      if (probe.absent) noteUnreadableUnits.delete(k);          // nothing on disk to lose — create it normally
      else if (probe.failed) noteBlocked = true;
      else {
        // Keep the folder's copy under the EXTERNAL-backup name, the one that means "the version that was in
        // the folder, kept before we wrote over it". NOT note.json.corrupt (ROUND-4/NSL-2): all we know is
        // that one read failed, and calling a healthy file corrupt sends the doctor hunting a phantom.
        // ROUND-5/R5-10 (CRITICAL): v91 put this copy under note.external-backup.json — a name
        // resolveConflictInner ALREADY writes unconditionally when the doctor picks "keep this session's
        // version". So an ordinary conflict resolution later overwrote the colleague's preserved note with
        // the doctor's own, and the only copy was gone while the file name still claimed to be the folder's
        // version. Its own name, and like .corrupt it is never overwritten by the rescue swap.
        // ROUND-6/R6-4: v92 wrote this name unconditionally. A frame can reach here twice — one hiccup
        // preserves a colleague's note, and a LATER hiccup on the same (now dirty) frame preserves whatever
        // note.json holds by then, straight over the first copy, which existed nowhere else. Same hazard
        ...
        catch (e) { noteBlocked = true; }
      }
      if (tok !== dsToken) return;
    }
    // ROUND-4/OT-3: v89 THREW here, which refused the annotation too — one Drive hiccup on note.json and the
    // doctor could not save the segments they had just drawn. The annotation always goes to disk. Only the
    // note waits, and the frame deliberately stays dirty so the retry queue comes back for it.
    let needNote = hasNote && !noteBlocked;
    ...
    const noteBk = needNote ? await backupNoteCorruptOnce(k, unit) : true;
    if (noteBk !== true) { noteBlocked = true; noteBlockReason = (noteBk === 'slots' ? 'slots' : 'read'); needNote = false; }   // ROUND-8/R8-1: only a FULL family is permanent
    if (tok !== dsToken) return;
    ...
    let annWriteMs = 0;   // ROUND-6: kept, because the noteBlocked path below must stamp THIS and not the unit-wide max
    // ROUND-8/R8-2: R7-5 correctly stopped inventing a timestamp from this machine's clock, but left the
    // read-back-failed case with NO account of our own write — and on the noteBlocked path State differs
    // from disk by the note, so the very next attempt declared the frame conflicted against the
    // annotation.json this session had just written, froze auto-save behind a chooser, and offered a
    // "keep the folder version" that empties the note box. The file is ours and it is right there: ask
    // again before giving up. Only if all three reads fail do we leave it unstamped (still never a fake).
    for (let i = 0; i < 3 && tok === dsToken && !annWriteMs; i++) {
      if (i) await new Promise(r => setTimeout(r, 120));
      const annMs = await annFileMtime(unit);
      if (annMs && tok === dsToken) { annWriteMs = annMs; ownWriteAt.set(k, annMs); lastSeenMtime.set(k, annMs); }
    }   // ROUND-3 FIX: the token was tested BEFORE the await and never after, so an in-flight write could stamp the NEW dataset's map
    if (needNote) {
      if (tok !== dsToken) return;   // ROUND-4 FIX: v89's B-2 guard was added to the annotation stamp above but not here — the note write itself could still land after a folder switch
      await FS.writeText(unit.handle, 'note.json', JSON.stringify(State.buildNote(caseId, unit.id), null, 2));
      if (tok !== dsToken) return;
      noteCorruptUnits.delete(k); noteUnreadableUnits.delete(k);   // the file on disk is valid JSON again, and readable
    }
    corruptUnits.delete(k); unimportedUnits.delete(k);   // the file on disk is valid JSON again (ROUND-4: unimportedUnits is corruptUnits' new sibling and was left add-only)
    // ROUND-5/R5-6 (CRITICAL): these stamps are UNIT-wide — unitDiskMtime is max(annotation, note), and
    // ownWriteAt/lastSeenMtime cover the whole frame. On the noteBlocked path we wrote annotation.json and
    // deliberately did NOT write note.json, so stamping the unit-wide MAX told reconcileUnitFromDisk and the
    // M4 guard that a colleague's LATER note.json was ours: reconcile returned 'kept-dirty' from its first
    // guard, before it ever compared content, and the chooser that would have kept both versions never
    // opened.
    // ROUND-6/R6-1: v92 fixed that by dropping the stamps ALTOGETHER, which over-corrected — the
    // annotation.json we really did write was then left unaccounted for, and because writtenAt is the
    // PERSISTED half, a reload declared the frame conflicted against our own file: the chooser popped on
    // every visit, Save-all said "a NEWER file is in the folder", and the only way out reset State to disk
    ...
    // ROUND-4/OT-3 second half: the ANNOTATION is on disk and must never be rolled back or retried away —
    // but the note is still only in memory, so this write is NOT a success. Hand the unit to the retry queue
    // exactly like any other partial failure: the frame stays dirty, the status line says "retrying" instead
    // of "Saved HH:MM", and the banner names the one file that is holding it up. `new Error` is deliberate —
  ...
  async function runAutoSave() {
    ...
    // ROUND-5/R5-7: writeFailedBanner is BANNER_CRITICAL (priority 2) and is NOT in the per-unit clear
    // list, so it buried the precise "note.json is the file holding this up" message and then followed the
    // doctor to every other frame until dismissed by hand. A blocked NOTE already raised its own banner,
    // and retryLater has already put "retrying" on the status line.
    // ROUND-6/R6-2: v92 returned early here, assuming retryLater had already written "retrying" — but on
    // the LAST rung retryLater drops the entry and returns false without touching the status line, so the
